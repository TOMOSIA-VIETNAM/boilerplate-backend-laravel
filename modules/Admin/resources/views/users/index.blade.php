                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <img class="bg-soft-primary rounded img-fluid avatar-40 me-3"
                                                src="{{ $user->getImageUrl($user->avatar) }}" alt="profile">
                                        </td>
                                        <td>{{ $user->name }}</td>
                                        <td>{{ $user->email }}</td>
                                            ...
                                            <div class="flex align-items-center list-user-action">
                                                ...
                                                @role('master_admin|admin')
                                                    <button class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal"
                                                        data-bs-target="#delete_modal{{ $user->id }}">
                                                        <span class="btn-inner">
                                                            <svg class="icon-20" width="20" viewBox="0 0 24 24"
                                                                fill="none" xmlns="http://www.w3.org/2000/svg"
                                                                stroke="currentColor">
                                                ...
                                                <x-admin::modal.basic-modal id="delete_modal{{ $user->id }}"
                                                    size="modal-md" method="POST" title="{{ __('Delete item') }}"
                                                    action="{{ route('admin.user.delete', $user->id) }}">
                                                    <div>
                                                        <div class="form-group">
                                                            <p for="">{{ __('Do you want to delete') }}
                                                                {{ $user->name }}?</p>
                                                        </div>
                                                    </div>
                                                    <x-slot:footer>
                                                        <div
                                                            class="tw-flex tw-items-center tw-justify-start tw-gap-x-5 tw-w-full">
                                                            <button class="btn btn-sm btn-danger"
                                                                type="submit">{{ __('Delete') }}</button>
                                                        </div>
                                                    </x-slot:footer>
                                                </x-admin::modal.basic-modal>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
